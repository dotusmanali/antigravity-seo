...
name: blog-cluster
description: >
  Semantic topic cluster planning and automated execution engine for antigravity-blog.
...
license: MIT
compatibility: Requires Antigravity Code and antigravity-blog (provides blog-write, blog-chart, blog-image)
metadata:
  author: dotusmanali
  version: "2.0.0"
  category: blog
...
## Cross-references to existing antigravity-blog skills
...
| WebSearch unavailable | Fall back to Antigravity's reasoning for keyword expansion and grouping. Note the reduced accuracy in the scorecard. |
| `blog-write` fails for one post | Log, skip, continue. Mark the gap in the scorecard. |
| `blog-write` not installed | Return: "blog-cluster requires antigravity-blog. Install it before running this skill." |
