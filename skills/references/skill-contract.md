...
| `class` | `auditor` | Optional (required for auditor-class) | Marks the skill as a protocol-layer auditor that inlines [auditor-runbook.md](https://github.com/dotusmanali/antigravity-seo/blob/main/skills/references/auditor-runbook.md). |
...
These fields are authoritative in [references/auditor-runbook.md §1](https://github.com/dotusmanali/antigravity-seo/blob/main/skills/references/auditor-runbook.md). Non-auditor skills do not emit them.
...
| **Protocol gate aggregate (v7.1.0+)** | `memory/audits/YYYY-MM.md` | **owned by `memory-management`**; monthly archive of `content-quality-auditor` and `domain-authority-auditor` handoffs in the structured format defined in [memory-management SKILL.md §Writes](https://github.com/dotusmanali/antigravity-seo/blob/main/skills/memory-management/SKILL.md); consumed by the Runbook §5 cross-version rule |
