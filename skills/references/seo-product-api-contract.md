# SEO Product API Contract
`/seo:auto` is the SEO/GEO pack-local Product API entry; its `--deep` flag (`/seo:auto --deep`) is the maximum-depth mode. `/seo` is an optional host alias for `/seo:auto`, not a physical command. Cross-pack capability selection belongs to the future `slash-antigravity` product repo; this pack must decline clearly non-SEO/GEO work instead of routing it into a fake SEO/GEO workflow. The five commands (`auto`, `research`, `create`, `audit`, `track`) remain the execution surface; scenario detail belongs in `evals/product-api-scenarios.md`.
## `/seo:auto`
...
- Do not redirect to `/seo:auto --deep` unless the user explicitly asks for maximum-depth, exhaustive, or stress-test mode; decline clearly non-SEO/GEO work with a pack-boundary note unless a verified slash-antigravity product registry provides another capability route.
...
New high-frequency or high-risk behavior must land as a scenario first, with a real `target_skill`, expected route, blocking inputs, and failure modes, before `/seo:auto` (including its `--deep` mode) changes.
