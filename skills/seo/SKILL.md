...
The community footer is disabled by default in Antigravity so API and client deliverables remain clean.
Append it only when the user explicitly asks for community attribution or when
`GEMINI_SEO_COMMUNITY_FOOTER=1` is set in the environment.
