...
This skill requires the DataForSEO MCP server configured in `mcp_config.json`.
...
- **MCP server not connected**: Report that DataForSEO MCP server is not configured. Suggest checking `mcp_config.json` and setting `DATAFORSEO_LOGIN`/`DATAFORSEO_PASSWORD` environment variables.
