...
| Error | Resolution |
|-------|-----------|
| MCP not configured | Check `mcp_config.json` and set `GOOGLE_AI_API_KEY` env var |
...
| Extension not installed | Check `mcp_config.json` and set `GOOGLE_AI_API_KEY` env var |
