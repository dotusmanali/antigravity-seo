...
license: MIT
compatibility: Requires Antigravity Code and Python 3.11+ for the sync script
metadata:
  author: dotusmanali
  version: "2.0.0"
  category: blog
...
> Framework and prompts (c) dotusmanali (dotusmanali@gmail.com), CC BY 4.0. Source: github.com/dotusmanali/flow

FLOW is an evidence-led operating model built for the AI-search era. Antigravity Blog
...
| `/blog flow sync` | Pull latest prompt files from github.com/dotusmanali/flow |
...
3. State that local-SEO prompts are excluded by design; point users to
   `antigravity-seo` (`/seo flow local`) if they need them.

### On `/blog flow sync`
1. Run: `python3 scripts/sync_flow_blog.py`.
...
`scripts/sync_flow_blog.py` pulls prompt files from github.com/dotusmanali/flow and
...
- `python3 scripts/sync_flow_blog.py`. Sync the latest version of every blog-relevant
  stage to disk and refresh the lockfile.
- `python3 scripts/sync_flow_blog.py --dry-run`. Report planned changes without
  writing.
- `python3 scripts/sync_flow_blog.py --ref <sha>`. Pin fetches to a specific FLOW
...
| `sync_flow_blog.py` network error | Display the script's stderr. Check rate limits with `gh api rate_limit` if `gh` is installed. |
| `sync_flow_blog.py` 403 after retry | Set `GITHUB_TOKEN` or run `gh auth login`, then retry. |
