...
license: MIT
compatibility: Requires antigravity-blog (blog-write). Optional integration with antigravity-seo (seo-hreflang) for richer hreflang validation.
metadata:
  author: dotusmanali
  version: "2.0.0"
  category: blog
...
> Adapted from `antigravity-blog-multilingual` by Chris Mueller (AI Marketing Hub
...
|-----------|--------|----------|
| `blog-write` | antigravity-blog (this plugin) | Yes |
| `blog-translate` | antigravity-blog (this plugin) | Yes |
| `blog-localize` | antigravity-blog (this plugin) | Yes (when `--localize` is on, default) |
| `seo-hreflang` | antigravity-seo (sibling plugin) | No, falls back to a self-contained generator |
...
from antigravity-seo is installed, delegate validation to it. Otherwise use the
...
- If antigravity-seo is installed, run `/seo hreflang multilingual/` for
  deeper validation.
```
...
| For deeper hreflang validation | `/seo hreflang <directory>` (antigravity-seo, optional) |

## Error Handling

| Scenario | Action |
|----------|--------|
| `blog-write` missing | Error: "This skill requires `blog-write`. Reinstall antigravity-blog." |
