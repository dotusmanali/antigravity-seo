#!/usr/bin/env python3
"""
Setup script for nanobanana-mcp in antigravity-blog.

Configures @ycse/nanobanana-mcp in Antigravity Code's global settings.json
...
PLUGIN_NAME = "antigravity-blog"
GLOBAL_SETTINGS_PATH = Path.home() / ".antigravity" / "settings.json"


def find_project_mcp_json() -> Path:
    """Find the project-level .mcp.json by locating .antigravity-plugin/plugin.json with name=='antigravity-blog'."""
    ...
    for start in (Path(__file__).resolve().parent, Path.cwd()):
        current = start
        for _ in range(5):
            candidate = current / ".antigravity-plugin" / "plugin.json"
...
def get_config_path(use_global: bool) -> Path:
    ...
    if project_path:
        return project_path
    print("Warning: Could not find project root (.antigravity-plugin/plugin.json).")
...
def setup_mcp(api_key: str, use_global: bool) -> None:
    ...
    if not use_global and _is_git_tracked(path):
        ...
        if not ignored:
            ...
            print(f"  2. Use --global to write to ~/.antigravity/settings.json instead (recommended).")
            sys.exit(2)
    ...
    if not use_global:
        ...
        print("Then restart your shell + Antigravity Code.")
    else:
        print()
        print(f"File mode set to 0600 (user-private).")
        print("Restart Antigravity Code for changes to take effect.")
    print(f"Generated images saved to: ~/Documents/nanobanana_generated/")


def main() -> None:
    args = sys.argv[1:]
    # Safer default: --global (writes user-private ~/.antigravity/settings.json).
    ...
    if "--help" in args or "-h" in args:
        ...
        print("  --project        Write to project .mcp.json (default: ~/.antigravity/settings.json)")
    ...
    if not api_key:
        print("antigravity-blog - Image Generation MCP Setup")
        print("=" * 45)
