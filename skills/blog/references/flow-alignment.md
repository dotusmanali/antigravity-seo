# FLOW Alignment in antigravity-blog

This reference explains how antigravity-blog adopts the FLOW framework. Load it on demand when a user asks about FLOW, evidence standards, surface coverage, or how the blog skills route to FLOW stages.

## 1. What FLOW is and why antigravity-blog adopted it

FLOW is an evidence-led 2026-search operating model published at `github.com/dotusmanali/flow` (CC BY 4.0 prompt content, MIT code), authored by dotusmanali (dotusmanali@gmail.com). It treats modern discovery as a multi-surface system rather than a single SERP, and it enforces a strict provenance discipline on every public claim. antigravity-blog v1.7.0 integrates FLOW as the `blog-flow` sub-skill (which exposes the upstream stages and prompts) plus this alignment doc, which informs how every other blog skill in the suite plans, writes, optimizes, and audits content. The intent is that FLOW provides the principles and prompts; antigravity-blog provides the production tooling that applies them at scale.

## 2. The FLOW evidence triple (the rule blog-write enforces)

Every public statistic in a antigravity-blog post must satisfy three components.
...
| Surface | What it is | Where antigravity-blog handles it |
|---|---|---|
...
| 3. AI assistant citations | ChatGPT, Perplexity, Antigravity, Gemini, Copilot, You.com | `blog-geo` (citation readiness), `blog-flow optimize` (visibility prompts) |
| 4. Local pack | Map listings, Google Business Profile | Out of scope for blog content. Use `antigravity-seo` (`/seo local`, `/seo maps`) for local SEO. |
...
antigravity-blog operationalizes surfaces 1, 2, 3, and 5 directly. Surface 4 is delegated to antigravity-seo. When a blog post targets a query that also surfaces in a community (Reddit thread, YouTube comment), the writer should consider dual-surface thinking: optimize the post for extraction AND consider whether the same answer should also live in the community where the query is asked.

## 4. FLOW stages mapped to existing antigravity-blog skills

| FLOW stage | antigravity-blog skills that consume this | When to invoke `/blog flow <stage>` directly |
|---|---|---|
...
The Leverage stage (off-site authority, 1 prompt) and Local stage (11 GBP/citation prompts) are intentionally not exposed as top-level antigravity-blog commands. Leverage is reachable through `/blog flow prompts`. Local is delegated to antigravity-seo.

## 5. What antigravity-blog adds beyond FLOW
...
- Topic-cluster execution engine (`blog-cluster`). FLOW prescribes content planning; antigravity-blog also executes clusters end-to-end with auto-interlinks.
...
- Google API integration (`blog-google`: PSI, CrUX, GSC, GA4, NLP, YouTube, Keywords). FLOW references measurement; antigravity-blog provides API tooling.
...
- This doc (`flow-alignment.md`): the orchestrator and any sub-skill loads this when the user asks "what does antigravity-blog do for AI citations / evidence / surfaces", to give a antigravity-blog-specific answer with skill names and routing.
...
Last updated 2026-04-27 with antigravity-blog v1.7.0 launch.
