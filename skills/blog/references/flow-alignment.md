...
FLOW is an evidence-led 2026-search operating model published at `github.com/dotusmanali/flow` (CC BY 4.0 prompt content, MIT code), authored by Daniel Agrici. It treats modern discovery as a multi-surface system rather than a single SERP, and it enforces a strict provenance discipline on every public claim. antigravity-blog v1.7.0 integrates FLOW as the `blog-flow` sub-skill (which exposes the upstream stages and prompts) plus this alignment doc, which informs how every other blog skill in the suite plans, writes, optimizes, and audits content. The intent is that FLOW provides the principles and prompts; antigravity-blog provides the production tooling that applies them at scale.
