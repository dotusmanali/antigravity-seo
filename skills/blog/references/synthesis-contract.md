...
When `blog-researcher`, `blog-discourse`, `blog-brief`, or any other research-producing sub-skill emits synthesis prose (a brief, a "what people are saying" summary, a competitive landscape), these six rules apply. They are mostly codifications of what antigravity-blog already enforces implicitly, plus a few that close real gaps.
...
**Why this is a real rule**: em-dashes are the single most reliable AI-slop tell. antigravity-blog's project memory already enforces this; the LAW codifies it as a synthesis-output contract.
...
**Why this matters**: inline links are the citation. They satisfy LAW 1's "no trailing Sources block" rule while still providing every reader with a clickable verification path. Markdown renderers (Antigravity Code, GitHub, most CMS exports) show only the link text and hide the URL, which keeps the prose clean.
...
The 6 LAWs above are adapted from `last30days-skill` v3.2.1's VOICE CONTRACT LAW section (Matt Van Horn, MIT, https://github.com/mvanhorn/last30days-skill). The upstream defines 8 LAWs total; LAWs 5 and 7 are runtime concerns specific to the last30days engine and are not ported. The 6 portable LAWs are re-grounded for antigravity-blog's general blog-research synthesis use case.
