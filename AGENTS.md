# Antigravity SEO: Universal SEO Analysis Skill
...
antigravity-seo/
  ...
  agents/seo-*.toml               # Antigravity specialist agent profiles
...
- Python dependencies install into `~/.gemini/skills/seo/.venv/`.
- All skills include a shared cache Step 0 and cache write guidance.
- New config paths use `~/.config/antigravity-seo/`; legacy `~/.config/antigravity-seo/` paths may be read only as migration fallback.
- Run `python -m pytest tests/` after changes.

## Key Principles

1. **Antigravity-first packaging**: skills, `.toml` agents, and `.gemini-plugin/plugin.json`.
...
Follow `/home/dotusmanali/Desktop/shipping-rules.md`: read first, write second, verify third. Keep changes scoped, preserve rollback paths, and verify claims with tests or direct inspection.
