...
def test_reference_count_coherence() -> None:
    ...
    docs/ARCHITECTURE.md and ANTIGRAVITY.md."""
    ...
    if arch_path.is_file():
        ...
        if m:
            ...
            assert declared_arch == actual, (
                f"docs/ARCHITECTURE.md says {declared_arch} references; filesystem has {actual}"
            )
        else:
            pytest.skip("ARCHITECTURE.md does not contain blog reference counts (merged repository)")

    antigravity_path = ROOT / "ANTIGRAVITY.md"
    if antigravity_path.is_file():
        antigravity_text = antigravity_path.read_text(encoding="utf-8")
        m = re.search(r"(\d+) reference docs", antigravity_text)
        if m:
            declared_antigravity = int(m.group(1))
            assert declared_antigravity == actual, (
                f"ANTIGRAVITY.md says {declared_antigravity}; filesystem has {actual}"
            )
        else:
            pytest.skip("ANTIGRAVITY.md does not contain reference doc count")
    else:
        pytest.skip("ANTIGRAVITY.md does not exist")
