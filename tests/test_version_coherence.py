...
def _read_plugin_version() -> str | None:
    with open(ROOT / ".antigravity-plugin" / "plugin.json", encoding="utf-8") as f:
...
def test_all_version_surfaces_aligned() -> None:
    ...
    versions = {
        ...
        ".antigravity-plugin/plugin.json": _read_plugin_version(),
...
def test_version_string_matches_semver_pattern() -> None:
    """The shared version must be valid semver (X.Y.Z optional pre-release/build meta)."""
    version = _read_pyproject_version()
    assert version is not None
    assert re.match(r"^\d+\.\d+\.\d+([+-]\S+)?$", version), (
        f"version {version!r} does not match semver pattern X.Y.Z[-pre][+build]"
    )
