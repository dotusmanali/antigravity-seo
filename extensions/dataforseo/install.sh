...
# DataForSEO Extension Installer for Antigravity SEO
...
main() {
    ...
    if [ -f "${SCRIPT_DIR}/../../skills/seo-dataforseo/SKILL.md" ]; then
        ...
        echo "  Run this script from the antigravity-seo repo: ./extensions/dataforseo/install.sh"
        exit 1
    fi
    ...
    if [ -f "${AGENT_SOURCE}" ] && [ "${AGENT_SOURCE}" != "${AGENT_DIR}/seo-dataforseo.toml" ]; then
        ...
        echo "  ✓ Antigravity TOML agent already installed"
    else
        echo "  ⚠  Antigravity TOML agent not found; reinstall the core Antigravity SEO suite if delegation is unavailable."
    fi
...
" || {
        ...
        echo "  Add the dataforseo server manually to ~/.gemini/settings.json"
