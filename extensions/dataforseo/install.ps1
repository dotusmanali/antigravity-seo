# DataForSEO Extension Installer for Antigravity SEO (Windows)
...
Write-Host "║   For Antigravity SEO                     ║" -ForegroundColor Cyan
Write-Host "════════════════════════════════════════" -ForegroundColor Cyan
Write-Host ""
...
if ($RepoRootCandidate -and (Test-Path (Join-Path $RepoRootCandidate.Path "skills\seo-dataforseo\SKILL.md"))) {
    ...
    $AgentSource = Join-Path $AntigravityRoot "agents\seo-dataforseo.toml"
    $FieldConfigSource = Join-Path $ScriptDir "field-config.json"
    ...
    Write-Host "  Run this script from the antigravity-seo repo."
    exit 1
}
...
if ($AgentSource -and (Test-Path $AgentSource) -and ((Resolve-Path $AgentSource).Path -ne $AgentTarget)) {
    ...
    Write-Host "  ✓ Antigravity TOML agent already installed" -ForegroundColor Green
} else {
    Write-Host "  ⚠  Antigravity TOML agent not found; reinstall the core Antigravity SEO suite if delegation is unavailable." -ForegroundColor Yellow
}
...
"@
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        Write-Host "  Add the dataforseo server manually to ~\.gemini\settings.json"
    }
} else {
...
Write-Host "  1. Restart Antigravity CLI"
