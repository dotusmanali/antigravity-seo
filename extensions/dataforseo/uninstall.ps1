# DataForSEO Extension Uninstaller for Antigravity SEO (Windows)
...
"@
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            Write-Host "  ⚠  Could not auto-remove MCP config. Remove 'dataforseo' from ~\.gemini\settings.json manually." -ForegroundColor Yellow
        }
    } else {
        Write-Host "  ⚠  Python not found. Remove 'dataforseo' from ~\.gemini\settings.json manually." -ForegroundColor Yellow
    }
}
