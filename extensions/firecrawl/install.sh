...
# Firecrawl Extension Installer for Antigravity SEO
...
main() {
    ...
    echo "║   For Antigravity SEO                     ║"
    echo "════════════════════════════════════════"
    echo ""

    # Check prerequisites
    if [ ! -d "${SEO_SKILL_DIR}" ]; then
        echo "x Antigravity SEO is not installed."
        echo "  Install it first: curl -fsSL https://raw.githubusercontent.com/Muhammad Usman Ali/antigravity-seo/main/install.sh | bash"
        exit 1
    fi
    echo "v Antigravity SEO detected"
    ...
    # Check if running from the repo, from an installed Antigravity SEO suite, or standalone.
    if [ -f "${SCRIPT_DIR}/../../skills/seo-firecrawl/SKILL.md" ]; then
        ...
        echo "  Run this script from the antigravity-seo repo: ./extensions/firecrawl/install.sh"
        exit 1
    fi
    ...
    if [ -f "${AGENT_SOURCE}" ] && [ "${AGENT_SOURCE}" != "${AGENT_DIR}/seo-firecrawl.toml" ]; then
        ...
        echo "  v Antigravity TOML agent already installed"
    else
        echo "  Warning: Antigravity TOML agent not found; reinstall the core Antigravity SEO suite if delegation is unavailable."
    fi
...
" || {
        ...
        echo "  Add the firecrawl-mcp server manually to ~/.gemini/settings.json"
    ...
    echo ""
    echo "Usage:"
    echo "  1. Restart Antigravity CLI"
    echo "  2. Run commands:"
