# Firecrawl Extension Installer for Antigravity SEO (Windows)
...
if ($AgentSource -and (Test-Path $AgentSource) -and ((Resolve-Path $AgentSource).Path -ne $AgentTarget)) {
    ...
    Write-Host "  v Antigravity TOML agent already installed" -ForegroundColor Green
} else {
    Write-Host "  Warning: Antigravity TOML agent not found; reinstall the core Antigravity SEO suite if delegation is unavailable." -ForegroundColor Yellow
}
