#!/usr/bin/env python3
"""
Setup script for the Banana image-generation MCP server in Antigravity.

Configures @ycse/nanobanana-mcp in Antigravity's settings.json
...
SETTINGS_PATH = Path(os.environ.get("GEMINI_HOME", Path.home() / ".gemini")) / "settings.json"
...
def load_settings() -> dict:
    """Load Antigravity settings.json."""
...
def save_settings(settings: dict) -> None:
    """Save Antigravity settings.json."""
...
def remove_mcp() -> None:
    ...
    if MCP_NAME in servers:
        ...
        print(f"Removed '{MCP_NAME}' from Antigravity settings.")
    else:
        print(f"'{MCP_NAME}' not found in settings.")


def setup_mcp(api_key: str) -> None:
    """Configure MCP server in Antigravity settings."""
