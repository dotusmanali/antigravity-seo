...
```bash
python3 ~/.gemini/skills/seo-image-gen/scripts/validate_setup.py
```

Or check manually:
1. `ls ~/.gemini/skills/seo-image-gen/SKILL.md`:skill file exists
2. `ls ~/.gemini/agents/seo-image-gen.toml`:agent file exists
3. `grep nanobanana ~/.gemini/settings.json`:MCP configured

## Common Issues

### "MCP tools not available"
- Restart Antigravity after installing the extension
- Verify your API key is valid at [aistudio.google.com](https://aistudio.google.com)
- Check `~/.gemini/settings.json` has the nanobanana-mcp entry
...
- Antigravity will suggest rephrased alternatives automatically
...
- Check the path returned by Antigravity after generation
