...
async def async_check_competitor_pages(domains: list[str], timeout: int = 10) -> dict[str, dict[str, bool]]:
    """Check for /blog and /pricing pages across multiple competitor domains concurrently."""
    results = {}
    async with aiohttp.ClientSession(
        ...
        for domain in domains:
            root = f"https://{domain}" if not domain.startswith("http") else domain
            tasks.append(check_paths(session, domain, root))
        
        check_results = await asyncio.gather(*tasks)
        for domain, res in check_results:
            results[domain] = res
    return results

async def check_paths(session, domain, root):
    res = {"has_blog": False, "has_pricing": False}
    for path in ["/blog", "/pricing"]:
        try:
            async with session.get(f"{root.rstrip('/')}{path}", allow_redirects=True) as resp:
                if resp.status == 200:
                    if path == "/blog": res["has_blog"] = True
                    if path == "/pricing": res["has_pricing"] = True
        except Exception:
            pass
    return domain, res
...
def generate_competitor_assets(target: str, timeout: int = DEFAULT_TIMEOUT) -> dict[str, Any]:
    ...
    # Prompt 4: Async check for competitor pages
    competitor_page_data = asyncio.run(async_check_competitor_pages(competitors, timeout=timeout))
    ...
    competitor_entries = [
        {
            ...
            "pages": competitor_page_data.get(competitor, {"has_blog": False, "has_pricing": False})
        }
