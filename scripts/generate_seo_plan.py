...
def resolve_plan_assets() -> Path:
    """Find seo-plan templates in repo and installed Antigravity skill layouts."""
    env_path = os.environ.get("GEMINI_SEO_PLAN_ASSETS")
    candidates = []
    if env_path:
        candidates.append(Path(env_path).expanduser())
    candidates.extend([
        ROOT / "skills" / "seo-plan" / "assets",  # repository checkout
        ROOT.parent / "seo-plan" / "assets",      # installed ~/.gemini/skills/seo sibling
    ])
...
def discover_site_pages(site_root: str) -> list[str]:
    ...
    try:
        ...
        response = session.get(f"{site_root}/sitemap.xml", timeout=20, headers={"User-Agent": "Mozilla/5.0 Antigravity-SEO-QA"})
