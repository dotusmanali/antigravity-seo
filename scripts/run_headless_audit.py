#!/usr/bin/env python3
"""
Run the Antigravity SEO full audit pipeline deterministically in headless environments.
...
from keyword_planner import get_keyword_volumes
...
async def async_crawl_sample_rows(
    site_root: str, sitemap_report: dict[str, Any], limit: int = 200, concurrency: int = 10
) -> list[dict[str, Any]]:
    """Fetch page URLs discovered in sitemaps and check title/meta description lengths asynchronously."""
    raw_urls = [site_root]
    ...
    connector = aiohttp.TCPConnector(ssl=False)  # Accept all certs for SEO crawl
    timeout = aiohttp.ClientTimeout(total=20)
    headers = {"User-Agent": "Mozilla/5.0 (compatible; AntigravitySEOHeadless/1.0; +https://github.com/dotusmanali/antigravity-seo)"}
        ...
        async with semaphore:
            try:
                async with session.get(url, allow_redirects=True) as response:
                    ...
                    page_data = parse_html(text, str(response.url)) if "html" in content_type else {}
                    return {
                ...
                return {"url": url, "status": None, "title_length": 0, "meta_description_length": 0}
...
def crawl_sample_rows(site_root: str, sitemap_report: dict[str, Any], limit: int = 200) -> list[dict[str, Any]]:
    """Synchronous wrapper for async_crawl_sample_rows."""
    return asyncio.run(async_crawl_sample_rows(site_root, sitemap_report, limit=limit))
...
def analyze_keywords(site_meta: dict[str, Any], timeout: int = 20) -> dict[str, Any]:
    """Analyze keywords for the site's industry and business type."""
    industry = site_meta.get("industry", "SEO")
    business_type = site_meta.get("business_type", "website")
    seed_keywords = [industry, f"{industry} {business_type}", f"best {industry}"]
    
    # Prompt 4: Use keyword_planner.py to fetch search volumes
    kw_data = get_keyword_volumes(seed_keywords)
    
    return {
        "cache_type": "keywords",
        "analyzed_at": now_iso(),
        "top_keywords": kw_data.get("keywords", [])[:5],
        "error": kw_data.get("error")
    }
...
def render_full_report(
    ...
    # Prompt 4: Keyword & Competitor report section
    kw_intelligence = category_results.get("keywords", {})
    competitor_intelligence = category_results.get("competitors", {})
    
    kw_rows = "\n".join([f"| {k['keyword']} | {k['avg_monthly_searches']} | {k['competition']} |" for k in kw_intelligence.get("top_keywords", [])])
    if not kw_rows: kw_rows = "| No keyword data | - | - |"
    
    comp_rows = "\n".join([f"| {c['brand']} | {'✅' if c.get('pages', {}).get('has_blog') else '❌'} | {'✅' if c.get('pages', {}).get('has_pricing') else '❌'} |" for c in competitor_intelligence.get("competitors", [])])
    if not comp_rows: comp_rows = "| No competitor data | - | - |"
...
This headless audit was generated by the deterministic Antigravity SEO pipeline without interactive orchestration. The report combines executable specialist outputs for technical SEO, content quality, structured data, image optimization, performance, sitemap quality, GEO readiness, and on-page SEO.
...
def synthesize_report_sections(
    ...
    if not provider:
        print("LLM synthesis skipped: no LLM provider configured", file=sys.stderr)
        return None
    ...
    try:
        print(f"Synthesizing report with {provider.__class__.__name__}...", file=sys.stderr)
        raw_content = provider.chat_completion([{"role": "user", "content": prompt}], max_tokens=8000)
        if not raw_content:
            return None
...
def run_audit_with_output_root(
    ...
    # Prompt 4: Integrate Keyword and Competitor analysis
    keywords = analyze_keywords(site_meta, timeout=timeout)
    competitors = generate_competitor_assets(response.url, timeout=timeout)
    ...
    write_json(ROOT / ".seo-cache" / "keywords.json", keywords)
    write_json(ROOT / ".seo-cache" / "competitors.json", competitors)
    write_json(
    ...
    write_json(output_dir / "keywords.json", keywords)
...
def main() -> int:
    """CLI entry point."""
    load_dotenv()
    parser = argparse.ArgumentParser(description="Run the deterministic Antigravity SEO full audit pipeline")
