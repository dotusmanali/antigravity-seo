...
def analyze_technical(url: str, timeout: int = DEFAULT_TIMEOUT) -> dict[str, object]:
    ...
    inp_value = performance.get("core_web_vitals", {}).get("INP", 0)
    ...
    hreflang_count = len(parse_data.get("hreflang", []))
    hreflang_score = 100
    if hreflang_count > 0:
        # Check if self-referencing hreflang exists
        self_ref = any(h["href"].rstrip("/") == response.url.rstrip("/") for h in parse_data["hreflang"])
        if not self_ref:
            hreflang_score -= 15
    
    # Pagination
    pagination = parse_data.get("pagination", {})
    has_pagination = pagination.get("next") or pagination.get("prev")
    ...
    if inp_value > 200:
        issues.append(f"Interaction to Next Paint (INP) is high ({inp_value}ms).")
        recommendations.append("Optimize main-thread tasks and reduce long-running JavaScript to improve INP.")
    if hreflang_count > 0 and hreflang_score < 100:
        issues.append("Internationalization (hreflang) tags are present but missing a self-reference.")
        recommendations.append("Ensure hreflang clusters always include a self-referencing tag for the current page.")
    if not has_pagination and "/page/" in response.url:
        issues.append("The page appears to be paginated but lacks rel=\"next/prev\" tags.")
        recommendations.append("Use rel=\"next\" and rel=\"prev\" to help search engines understand the relationship between paginated components.")
