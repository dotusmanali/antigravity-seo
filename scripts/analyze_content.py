...
from seo_pipeline_utils import build_session, validate_public_url, classify_label
...
def fetch_html(url: str, timeout: int) -> tuple[requests.Response | None, str | None]:
    ...
    try:
        ...
        response = session.get(
            ...
            headers={"User-Agent": "Mozilla/5.0 Antigravity-SEO-QA"},
        )
...
def analyze_content(url: str, timeout: int = DEFAULT_TIMEOUT) -> dict[str, Any]:
    ...
    footer_trust = any(token in footer_text for token in ["about", "contact", "privacy", "terms"])

    # Specialized terminology (Expertise)
    industry_terms = {
        "saas": ["api", "integration", "deployment", "scalability", "uptime", "latency"],
        "ecommerce": ["sku", "inventory", "logistics", "fulfillment", "conversion"],
        "agency": ["roi", "kpi", "strategy", "campaign", "optimization"],
        "local": ["service area", "licensed", "insured", "emergency"],
    }
    
    # Simple industry detection for terms check
    detected_industry = "generic"
    if any(token in soup.get_text().lower() for token in ["pricing", "demo", "saas"]):
        detected_industry = "saas"
    elif any(token in soup.get_text().lower() for token in ["cart", "shop", "checkout"]):
        detected_industry = "ecommerce"

    expertise_terms_found = sum(1 for term in industry_terms.get(detected_industry, []) if term in soup.get_text().lower())
    ...
    if "strong" == classify_label(ai_readiness_numeric) and explicit_author_markup is False:
    ...
    return {
        ...
        "ai_citation_readiness": classify_label(ai_readiness_numeric),
        "issues": issues,
        "page_type": page_type,
