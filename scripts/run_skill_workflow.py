#!/usr/bin/env python3
"""
Run a Antigravity SEO skill deterministically and write standard artifacts.
...
def antigravity_settings_path() -> Path:
    """Return the user-level Antigravity settings path."""
...
def configured_mcp_server(name: str, required_env: list[str] | None = None) -> dict[str, Any]:
    """Return sanitized MCP server configuration status from Antigravity settings."""
    settings_path = antigravity_settings_path()
    try:
...
def run_specialist(skill: str, target: str, output_root: Path | None = None) -> dict[str, Any]:
    ...
    if skill == "seo-google":
        ...
        recommendations = ["Create `~/.config/antigravity-seo/google-api.json` or set the documented environment variables."]
    ...
    if skill in {"seo-dataforseo", "seo-firecrawl", "seo-image-gen", "seo-maps"}:
        ...
        if mcp_status["configured"]:
            ...
            issues = []
            recommendations = ["Restart Antigravity CLI so the configured MCP server is loaded before requesting live data."]
        else:
        ...
        summary = [setup[skill]]
        if mcp_status["configured"]:
            summary.append(f"Antigravity settings include `{server_name}` with required environment keys present.")
        else:
            summary.append(f"Antigravity settings do not include a complete `{server_name}` MCP configuration.")
    ...
    if skill in {"seo-local", "seo-cluster", "seo-sxo", "seo-ecommerce", "seo-flow"}:
        labels = {
            "seo-local": "Local SEO analysis is available through the Antigravity skill workflow; live GBP/map data requires optional integrations.",
        ...
        return run_capability_summary(
            ...
            [labels[skill]],
            [],
            ["Run the specialist skill in Antigravity for evidence collection and recommendations."],
            [],
            output_root,
        )
...
def main() -> int:
    """CLI entry point."""
    parser = argparse.ArgumentParser(description="Run a Antigravity SEO skill deterministically")
