#!/usr/bin/env python3
"""
Validate Antigravity SEO demo readiness without printing provider secrets.
...
def antigravity_settings_path() -> Path:
    """Resolve Antigravity settings.json from GEMINI_HOME or the default home path."""
    return Path(os.environ.get("GEMINI_HOME", Path.home() / ".gemini")) / "settings.json"


def load_settings(path: Path) -> dict[str, Any]:
    """Load Antigravity settings, returning an empty config if the file is absent."""
    if not path.exists():
        return {}
...
def settings_check(settings_path: Path, settings: dict[str, Any]) -> dict[str, Any]:
    ...
    if os.name == "nt":
        # On Windows, POSIX permission bits for group/other are not natively supported
        mode_ok = exists
    else:
...
def build_report(
    ...
    serp_keyword: str = "antigravity seo",
) -> dict[str, Any]:
    ...
    report: dict[str, Any] = {
        ...
        "notes": [
            "Restart Antigravity CLI after MCP settings changes so demo sessions load the configured servers.",
...
def render_text(report: dict[str, Any]) -> str:
    ...
    lines = [
        "Antigravity SEO Demo Readiness",
        "=" * 24,
...
def main() -> int:
    """CLI entry point."""
    parser = argparse.ArgumentParser(description="Validate Antigravity SEO demo wiring without printing secrets")
    ...
    parser.add_argument("--serp-keyword", default="antigravity seo", help="Keyword for --live-serp")
