#!/usr/bin/env python3
"""
Google API credential management for Antigravity SEO.
...
CONFIG_PATH = os.path.expanduser("~/.config/antigravity-seo/google-api.json")
TOKEN_PATH = os.path.expanduser("~/.config/antigravity-seo/oauth-token.json")
LEGACY_CONFIG_PATH = os.path.expanduser("~/.config/antigravity-seo/google-api.json")
...
def load_config() -> dict:
    ...
    Reads ~/.config/antigravity-seo/google-api.json first. Any missing fields
    ...
    # Load from the Antigravity config file, with read-only fallback for old Antigravity SEO installs.
...
def main():
    parser = argparse.ArgumentParser(
        description="Google API credential management for Antigravity SEO"
    )
    parser.add_argument(
        "--check",
        nargs="?",
