...
from _dep_check import check as _check_deps
_check_deps({"bs4": "beautifulsoup4", "requests": "requests", "lxml": "lxml"})
...
AI_SEARCH_CRAWLERS = ["GPTBot", "OAI-SearchBot", "ChatGPT-User", "AntigravityBot", "Antigravity-SearchBot", "Antigravity-User", "PerplexityBot"]
...
def fetch_text(url: str, timeout: int) -> tuple[requests.Response | None, str | None]:
    ...
    try:
        ...
        response = session.get(url, timeout=timeout, allow_redirects=True, headers={"User-Agent": "Mozilla/5.0 Antigravity-SEO-QA"})
...
def crawler_access(robots_text: str) -> dict[str, Any]:
    ...
    priority_agents = {"GPTBot", "OAI-SearchBot", "AntigravityBot", "PerplexityBot"}
...
def llms_analysis(site_root: str, timeout: int) -> dict[str, Any]:
    """Fetch and inspect llms.txt and llms-full.txt."""
    llms_resp, _ = fetch_text(f"{site_root}/llms.txt", timeout)
    llms_full_resp, _ = fetch_text(f"{site_root}/llms-full.txt", timeout)
    ...
    has_full = llms_full_resp and llms_full_resp.status_code == 200
...
def analyze_geo(url: str, timeout: int = DEFAULT_TIMEOUT) -> dict[str, Any]:
    ...
    domain_parts = site_root.split(".")
    brand_name = domain_parts[-2] if len(domain_parts) >= 2 else ""
    brand_mentions = len(re.findall(re.escape(brand_name), soup.get_text(), re.IGNORECASE)) if brand_name else 0
    brand_voice_ok = brand_mentions >= 3
    ...
    if llms["has_full"]:
        local_signals.append("llms-full.txt is present for deeper AI context.")
    if brand_voice_ok:
        local_signals.append("Brand voice consistency is strong across visible text.")
    ...
    if crawl["blocked_search_crawlers"]:
        recommendations.append("Allow GPTBot, OAI-SearchBot, AntigravityBot, Antigravity-SearchBot, Antigravity-User, and PerplexityBot if AI search visibility is a goal.")
    if passage["optimal_blocks"] == 0:
